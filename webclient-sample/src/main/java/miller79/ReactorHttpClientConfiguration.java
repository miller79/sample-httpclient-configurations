...
class ReactorHttpClientConfiguration {
    ...
    /**
     * Creates a {@link ClientHttpConnectorBuilderCustomizer} with custom Reactor
     * Netty configuration.
     * 
     * <p>
     * This customizer is automatically applied by Spring Boot 3.4+ to configure all
     * WebClient instances with custom Reactor Netty HTTP client settings. It
     * configures:
     * <ul>
     * <li><b>Connection Provider:</b> Manages connection pool with configurable
     * size and lifecycle</li>
     * <li><b>Reactor Resource Factory:</b> Shared event loop and connection
     * provider resources</li>
     * <li><b>HTTP Client Factory:</b> Creates HttpClient with TCP keep-alive and
     * channel options</li>
     * </ul>
     * 
     * <p>
     * <b>Connection Pool Management:</b><br>
     * The {@link ConnectionProvider} manages a pool of connections that can be
     * reused across multiple HTTP requests. Key behaviors:
     * <ul>
     ...
     * <li><b>maxLifeTime:</b> Maximum lifetime of any connection (prevents stale
     * connections)</li>
     * <li><b>evictInBackground:</b> Periodic cleanup interval for idle
     * connections</li>
     * </ul>
     * 
     * <p>
     * <b>TCP Keep-Alive Strategy:</b><br>
     * TCP keep-alive is configured at the Netty channel level using Epoll options
     * (Linux-specific). The kernel sends keep-alive probes after
     * {@code tcpKeepIdle} seconds of inactivity, then sends {@code tcpKeepCount}
     * probes every {@code tcpKeepInterval} seconds. If all probes fail, the
     * connection is closed.
     * 
     * <p>
     * <b>Epoll vs NIO:</b><br>
     * This configuration uses {@link EpollChannelOption} which provides native
     * socket options on Linux. For cross-platform support, you may need to
     * conditionally use {@code ChannelOption} instead.
     * 
     * <p>
     * <b>Response Timeout:</b><br>
     * Note that this configuration does not set a response timeout. Spring Boot
     * 3.4+ provides separate mechanisms for configuring response timeouts on
     * WebClient instances if needed. Unlike Apache HttpClient's multiple timeout
     * layers, Reactor Netty primarily uses a single response timeout that applies
     * to the entire request/response cycle.
     * 
     * <p>
     * <b>Auto-Configuration:</b><br>
     * Spring Boot automatically applies this customizer to all WebClient.Builder
     * instances in the application context using the fluent builder API.
     * 
     * @param resources Spring's ReactorResourceFactory for shared event loop
     *                  management
     * @return a configured ClientHttpConnectorBuilderCustomizer for Reactor Netty
     * @see ClientHttpConnectorBuilderCustomizer for Spring Boot 3.4+ customization
     *      pattern
